...
public class HeartBeat {
    public Map<String, Integer> slave_ip_list;
    public int k;

    public HeartBeat() {
        // initialize your data structure here
        slave_ip_list = new HashMap<String, Integer>();
    }
    ...
    public void ping(int timestamp, String slave_ip) {
        // Write your code here
        if(!slave_ip_list.containsKey(slave_ip))
            return;
        slave_ip_list.put(slave_ip, timestamp);
    }
    ...
    public List<String> getDiedSlaves(int timestamp) {
        ...
        for(String slave : slave_ip_list.keySet()){
            if(timestamp - slave_ip_list.get(slave) >= 2 * k){
                diedSlaves.add(slave);
            }
        }
        return diedSlaves;
    }
